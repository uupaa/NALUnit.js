...
function PPS(nalUnitObject,  // @arg NALUnitObject - { nal_ref_idc, nal_unit_type, nal_unit_size, index, data, NAL_UNIT_TYPE }
    ...
    pic_parameter_set_rbsp.call(this, bitView, parameterSet);
...
// --- implements ------------------------------------------
function pic_parameter_set_rbsp(bitView, parameterSet) {
    ...
    if (!bitView["EOS"]) { // if (more_rbsp_data()) { ... }
        ...
        if (pic_scaling_matrix_present_flag) {
            ...
            var sps_id = this["seq_parameter_set_id"];
            var sps    = parameterSet["getSPS"](sps_id);

            for (var i = 0; i < 6 + ( (sps["chroma_format_idc"] !== 3 ) ? 2 : 6 ) * transform_8x8_mode_flag; ++i) {
